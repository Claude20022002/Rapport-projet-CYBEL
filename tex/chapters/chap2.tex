...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\textwidth]{images/cybel.jpeg}
...
\paragraph{Couche de présentation et d'interaction (couche supérieure):}
Au sommet de l'architecture se trouve la couche de présentation, qui regroupe
les interfaces utilisateur du robot. On y distingue trois composants :
\textbf{l'application propriétaire Android}, boîte noire hors périmètre ;
\textbf{l'interface opérateur web (CYBEL)}, développée dans le projet pour le
contrôle et la supervision ; et \textbf{l'interface visiteur (mode kiosque)},
qui offre une interaction simplifiée en contexte public. Cette couche sert de
point d'entrée homme-machine et s'appuie sur des échanges réseau vers les
niveaux inférieurs, notamment HTTP et WebSocket. Côté Android, un module de
\textbf{synthèse vocale (TTS natif)} et un mode \textbf{WebView kiosque}
complètent les interfaces embarquées.

\paragraph{Couche applicative et API (couche intermédiaire):}
La deuxième couche correspond à la logique applicative centrale, incarnée
par l'\textbf{API CYBEL en FastAPI}. Elle orchestre les échanges entre
interfaces et robot en exposant des endpoints REST et des flux WebSocket.
Cette API utilise soit un \textbf{SDK Python réel} pour la communication
directe avec le robot, soit un \textbf{SDK Python simulé (\textit{mock})}
pour le développement et les tests, tous deux basés sur une interface
abstraite commune (\texttt{RobotBackend}).

\paragraph{Couche de communication et de services réseau:}
La troisième couche regroupe les interfaces réseau du robot et les services
qui exposent ses fonctionnalités internes, notamment \textbf{rosbridge}
(WebSocket, port 9090), \textbf{rosapi}, \textbf{MQTT} (port 1883) et
\textbf{ADB} (Android Debug Bridge). Elle fait le lien entre la logique
applicative CYBEL et le système robotique réel.
...
distinct (\texttt{172.16.0.0/16}).
\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{diagramme/architecture_globale.jpeg}
...
CYBEL remplace l'application propriétaire dans les couches supérieures,
ajoute des fonctions et s'appuie sur FastAPI, le SDK Python, rosbridge et MQTT
pour commander le robot, tout en restant compatible avec l'infrastructure
existante.
...
Rétro-ingénierie de la logique métier de l'application Android propriétaire
d'accueil (une inspection ciblée de l'outil de déploiement constructeur, via
JADX, a néanmoins été nécessaire: voir chapitre 5) \\
\hline
...
\begin{center}
\begin{tabular}{|p{0.10\textwidth}|p{0.67\textwidth}|p{0.15\textwidth}|}
\hline
...
\begin{itemize}
    ...
    \item \textbf{Contraintes de propriété intellectuelle} : la logique métier
    de l'application propriétaire d'accueil n'est pas rétro-ingénierée ;
    l'analyse repose principalement sur l'observation du trafic réseau et
    l'introspection des interfaces standard (ROS, MQTT). Une inspection
    ciblée, via JADX, de l'outil constructeur de déploiement (\textit{Deployment
    Tool}) a néanmoins été nécessaire pour retrouver les coordonnées non
    documentées des points d'intérêt du laboratoire (voir chapitre 5).
\end{itemize}
...
Envoi d'une commande de navigation suivie d'un changement cohérent du champ
de statut \texttt{nav\_status} (détaillé au chapitre 5) \\
\hline
...
Face à ce besoin, la solution proposée consiste en une plateforme logicielle
